  ...
  if (arrNum.length % 2 === 0) {
    for (let i = 0; i < arrNum.length; i += 1) {
      if (i % 2 === 0) {
        arrNum[i] *= 2;
        if (arrNum[i] > 9) {
          arrNum[i] = arrNum[i].toString().split('').map((num) => Number(num)).reduce((a, b) => a + b);
        }
      } else {
        arrNum[i] -= 0;
      }
...
function getDigitalRoot(num) {
  ...
  return (sum < 10) ? sum : sum.toString().split('').map((item) => Number(item)).reduce((a, b) => a + b);
}
...
function isBracketsBalanced(str) {
  ...
  let pairsBrack = {};
  pairsBrack = Object.fromEntries((bracketsConfig).map(([key, value]) => [value, key]));
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openBrack.includes(currentSymbol)) {
      ...
      const topElement = stack1[stack1.length - 1];
